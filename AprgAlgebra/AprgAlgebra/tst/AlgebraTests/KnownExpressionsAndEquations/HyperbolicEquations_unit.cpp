...
{
    EXPECT_EQ("((e)^x)", convertToString(getEToTheX("x")));
}
...
{
    EXPECT_EQ("tanh(x) = ((((e)^x)-((e)^-1[x]))/(((e)^x)+((e)^-1[x])))", convertToString(getHyperbolicTangentDefinition("x")));
}
...
{
    EXPECT_EQ("coth(x) = ((((e)^x)+((e)^-1[x]))/(((e)^x)-((e)^-1[x])))", convertToString(getHyperbolicCotangentDefinition("x")));
}
...
{
    EXPECT_EQ("((e)^-1[x]) = (cosh(x)-sinh(x))", convertToString(getEToTheNegativeXFromHyperbolicFunctionsEquation("x")));
}
