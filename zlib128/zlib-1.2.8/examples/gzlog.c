...
#define BAIL(n)                                                                     \
    do {                                                                            \
        if (n == gzlog_bail && gzlog_count-- == 0) longjmp(gzlog_jump, gzlog_bail); \
    } while (0)
#else
#define BAIL(n)
#endif
...
local int log_lock(struct log *log) {
    ...
    close(fd);
    if (stat(log->path, &st) == 0) log->lock = st.st_mtime;
    return 0;
}
...
local int log_check(struct log *log) {
    ...
    strcpy(log->end, ".lock");
    if (stat(log->path, &st) || st.st_mtime != log->lock) return 1;
    log_touch(log);
    return 0;
}
...
local int log_head(struct log *log) {
    ...
    if (lseek(log->fd, 0, SEEK_SET) < 0 || read(log->fd, buf, HEAD + EXTRA) != HEAD + EXTRA ||
...
local int log_last(struct log *log, int last) {
    ...
    buf[0] = 0;
    if (back != 8 && (lseek(log->fd, log->last - len, SEEK_SET) < 0 || read(log->fd, buf, 1) != 1)) return -1;
...
local int log_append(struct log *log, unsigned char *data, size_t len) {
    ...
    while (len) {
        ...
        if (put > len) put = (uint)len;
        if (put) {
            if (write(log->fd, data, put) != put) return -1;
            BAIL(1);
    ...
    PUT4(buf, log->tcrc);
    PUT4(buf + 4, log->tlen);
    if (write(log->fd, buf, 8) != 8 || (end = lseek(log->fd, 0, SEEK_CUR)) < 0 || ftruncate(log->fd, end)) return -1;

    /* write the extra field, marking the log file as done, delete .add file */
    if (log_mark(log, NO_OP)) return -1;
    strcpy(log->end, ".add");
...
local int log_compress(struct log *log, unsigned char *data, size_t len) {
    ...
    if (len) {
        ...
        strm.zfree = Z_NULL;
        strm.opaque = Z_NULL;
        if (deflateInit2(&strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED, -15, 8, Z_DEFAULT_STRATEGY) != Z_OK) return -2;
        ...
        if (fd >= 0) {
            ...
            if (dict) deflateSetDictionary(&strm, buf, (uint)dict);
        }
        log_touch(log);
        ...
        if ((log->first = lseek(log->fd, -1, SEEK_CUR)) < 0 || read(log->fd, buf, 1) != 1) return -1;
        log->first++;
        if (*buf) {
            log->back = 1;
    ...
    PUT4(buf, log->tcrc);
    PUT4(buf + 4, log->tlen);
    if (log_last(log, 1) || write(log->fd, buf, 8) != 8 || (end = lseek(log->fd, 0, SEEK_CUR)) < 0 ||
        ftruncate(log->fd, end))
        return -1;
    BAIL(6);

    /* mark as being in the replace operation */
    if (log_mark(log, REPLACE_OP)) return -1;
...
local void log_log(struct log *log, int op, char *record) {
    ...
    fprintf(
        ...
        op == APPEND_OP ? "append" : (op == COMPRESS_OP ? "compress" : "replace"), record);
    fclose(rec);
    return;
}
...
int gzlog_compress(gzlog *logd) {
    ...
    /* check arguments */
    if (log == NULL || strcmp(log->id, LOGID)) return -3;

    /* see if we lost the lock -- if so get it again and reload the extra
       field information (it probably changed), recover last operation if
       necessary */
    if (log_check(log) && log_open(log)) return -1;

    /* create space for uncompressed data */
    len = ((size_t)(log->last - log->first) & ~(((size_t)1 << 10) - 1)) + log->stored;
    if ((data = malloc(len)) == NULL) return -2;

    /* do statement here is just a cheap trick for error handling */
    do {
        /* read in the uncompressed data */
        if (lseek(log->fd, log->first - 1, SEEK_SET) < 0) break;
        next = 0;
        while (next < len) {
            if (read(log->fd, buf, 5) != 5) break;
            block = PULL2(buf + 1);
            if (next + block > len || read(log->fd, (char *)data + next, block) != block) break;
            next += block;
        }
        ...
        log->last = log->first;
        log->stored = 0;
        if (log_mark(log, COMPRESS_OP)) break;
        BAIL(7);
...
int gzlog_write(gzlog *logd, void *data, size_t len) {
    ...
    if (log_mark(log, APPEND_OP)) return -1;
    BAIL(8);

    /* append data (clears mark) */
    if (log_append(log, data, len)) return -1;

    /* check to see if it's time to compress -- if not, then done */
    if (((log->last - log->first) >> 10) + (log->stored >> 10) < TRIGGER) return 0;
...
int gzlog_close(gzlog *logd) {
    ...
    if (log->path != NULL) free(log->path);
    strcpy(log->id, "bad");
    free(log);
    return 0;
}
