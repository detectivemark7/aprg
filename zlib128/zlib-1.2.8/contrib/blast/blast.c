...
local int decomp(struct state *s) {
    ...
    do {
        if (bits(s, 1)) {
            ...
            dist += bits(s, symbol);
            dist++;
            if (s->first && dist > s->next) return -3; /* distance too far back */
...
int blast(blast_in infun, void *inhow, blast_out outfun, void *outhow) {
    ...
    if (err != 1 && s.next && s.outfun(s.outhow, s.out, s.next) && err == 0) err = 1;
    return err;
}
...
local int outf(void *how, unsigned char *buf, unsigned len) { return fwrite(buf, 1, len, (FILE *)how) != len; }
