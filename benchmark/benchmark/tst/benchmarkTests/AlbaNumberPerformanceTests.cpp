...
static void BM_AlbaNumberFractionCreation(benchmark::State & state)
{
    ...
    while(state.KeepRunning())
    {
        ...
        AlbaNumber randomNumber(AlbaNumber::createFraction(randomizer.getRandomInteger(), static_cast<uint32_t>(randomizer.getRandomInteger())));
        escape(&randomNumber);
    }
}
...
static void BM_AlbaNumberComplexNumberCreation(benchmark::State & state)
{
    ...
    while(state.KeepRunning())
    {
        ...
        AlbaNumber randomNumber(AlbaNumber::createComplexNumber(randomizer.getRandomFloatingPointValue(), randomizer.getRandomFloatingPointValue()));
        escape(&randomNumber);
    }
}
