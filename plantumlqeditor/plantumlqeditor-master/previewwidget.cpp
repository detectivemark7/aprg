...
PreviewWidget::PreviewWidget(QWidget *parent) : QWidget(parent), m_mode(NoMode), m_zoomScale(ZOOM_ORIGINAL_SCALE) {
...
void PreviewWidget::zoomIn() {
    ...
    if (new_scale > MAX_ZOOM_SCALE) new_scale = MAX_ZOOM_SCALE;
...
void PreviewWidget::zoomOut() {
    ...
    if (new_scale < MIN_ZOOM_SCALE) new_scale = MIN_ZOOM_SCALE;
    setZoomScale(new_scale);
}
...
void PreviewWidget::zoomImage() {
    if (m_mode == PngMode) {
        if (m_zoomScale == ZOOM_ORIGINAL_SCALE) {
            ...
            float zoom = float(m_zoomScale) / ZOOM_ORIGINAL_SCALE;
            m_zoomedImage = m_image.scaled(
                m_image.width() * zoom, m_image.height() * zoom, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
        }
    }
}
