...
public:
    LineNumberArea(TextEdit *editor) : QWidget(editor) { this->editor = editor; }
...
TextEdit::TextEdit(QWidget *parent)
    : QPlainTextEdit(parent), _indentSize(4), _indentWithSpace(false), _autoIndent(true) {
    setIndentSize(4);
    ...
    switch (e->key()) {
        ...
        case Qt::Key_Enter: {
            ...
            if (_autoIndent) {
                ...
                while ((i + 1) < data.length() && data.at(i + 1).isSpace() && data.at(i + 1) != '\n' &&
                       data.at(i + 1) != '\r') {
...
void TextEdit::updateLineNumberAreaWidth(int /*newBlockCount*/) { setViewportMargins(lineNumberAreaWidth(), 0, 0, 0); }
