...
public:
    ...
    RangeQueryWithQuadraticAuxiliarySize(Values const& valuesToCheck, Function const& selector)
        : m_partialValueMatrix(valuesToCheck.size(), valuesToCheck.size(), 0U), m_function(selector) {
    ...
    Value getValueOnInterval(Index const start, Index const end) const {
        ...
        Value result{};
        if (start <= end && start < m_partialValueMatrix.getNumberOfColumns() &&
            end < m_partialValueMatrix.getNumberOfRows()) {
