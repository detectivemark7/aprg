...
public:
            ...
            Index const bottomRightY) const
    {
        ...
        Value positivePart = m_accumulator(getPartialResultAt(bottomRightX, bottomRightY), getPartialResultAt(topLeftX-1, topLeftY-1));
        Value negativePart = m_accumulator(getPartialResultAt(topLeftX-1, bottomRightY), getPartialResultAt(bottomRightX, topLeftY-1));
...
private:
    ...
    {
        Value result{};
        if(x>=0 && x<static_cast<int>(m_partialResults.getNumberOfColumns())
                && y>=0 && y<static_cast<int>(m_partialResults.getNumberOfRows()))
        {
            result=m_partialResults.getEntryConstReference(static_cast<unsigned int>(x), static_cast<unsigned int>(y));
        }
        return result;
    }
