...
public:
    ...
    MaximumSubArraySum(Values const& valuesToCheck)
    ...
    Value getMaximumSubArraySum()
    {
        ...
        // Thus, we can solve the problem efficiently by calculating the maximum subarray sum or each ending position from left to right.

        // This linear-time algorithm is attributed to J. B. Kadane, and the algorithm is sometimes called Kadane’s algorithm.
