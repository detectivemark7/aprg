...
private:
    void sort(Values& valuesToSort, unsigned int const lowestIndex, unsigned int const highestIndex) const {
        if (lowestIndex < highestIndex) {
            unsigned int indexForPivotValue =
                getIndexForPivotValue(valuesToSort, lowestIndex, highestIndex, m_pivotType);
            auto pivotValue = valuesToSort.at(indexForPivotValue);
            if (indexForPivotValue != lowestIndex) {
                std::swap(
                    valuesToSort[indexForPivotValue],
                    valuesToSort[lowestIndex]);  // put it as lowest index so it would not be affected
            }

            unsigned int boundaryIndexForLessThan = lowestIndex, i = lowestIndex + 1,
                         boundaryIndexForGreaterThan = highestIndex;
            while (i <= boundaryIndexForGreaterThan)  // i moves within the boundaries
            {
                if (valuesToSort.at(i) < pivotValue) {
                    ...
                    std::swap(
                        valuesToSort[boundaryIndexForLessThan++],
                        valuesToSort[i++]);  // i is moved here as well (to keep i within the boundary)
            ...
            if (boundaryIndexForLessThan > 0)  // prevent negative index
            {
                sort(
                    ...
                    boundaryIndexForLessThan - 1);  // sort items that are less than the pivotValue
            }
            sort(
                valuesToSort, boundaryIndexForGreaterThan + 1,
                highestIndex);  // sort items that are greater than the pivotValue
