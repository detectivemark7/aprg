...
// Step 2) As long as there exists a vertex u that belongs to the current tour but that has adjacent edges not part of
// the tour, start another trail from u, following unused edges until returning to u, and join the tour formed in this
...
// (finding unused edges exiting each vertex, finding a new starting vertex for a tour, and connecting two tours that
// share a vertex) may be performed in constant time each, so the overall algorithm takes linear time O(E).

// This algorithm may also be implemented with a deque.
// Because it is only possible to get stuck when the deque represents a closed tour, one should rotate the deque by
// removing edges from the tail and adding them to the head until unstuck, and then continue until all edges are
// accounted for. This also takes linear time O(E), as the number of rotations performed is never larger than |E|/
