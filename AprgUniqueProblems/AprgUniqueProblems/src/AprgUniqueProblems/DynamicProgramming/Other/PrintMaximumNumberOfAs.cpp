...
PrintMaximumNumberOfAs::PrintMaximumNumberOfAs(
        Count const numberOfPresses)
    : m_numberOfPresses(numberOfPresses)
{}
...
PrintMaximumNumberOfAs::Count PrintMaximumNumberOfAs::getMaxCountOfAOnScreenUsingNaiveRecursion() const
{
    ...
    auto it = max_element(countDetails.cbegin(), countDetails.cend(), [](
                          CountDetail const& first, CountDetail const& second)
    {
...
PrintMaximumNumberOfAs::Count PrintMaximumNumberOfAs::getMaxCountOfAOnScreenUsingMemoizationDP() const
{
    ...
    CountDetailMatrix savedCounts(
                m_numberOfPresses+1,
                static_cast<Count>(PressType::CtrlV)+1,
...
PrintMaximumNumberOfAs::Count PrintMaximumNumberOfAs::getMaxCountOfAOnScreenUsingIterativeDP() const
{
    ...
    for(Count remainingNumberOfPresses=1; remainingNumberOfPresses<=m_numberOfPresses; remainingNumberOfPresses++)
    {
        for(Count pressTypeValue=static_cast<Count>(PressType::TypeA);
            pressTypeValue<=static_cast<Count>(PressType::CtrlV);
            pressTypeValue++)
        {
            ...
            savedCounts.setEntry(
                        ...
                        getBestDetailBasedOnPreviousDetails(
                            previousDetails,
                            remainingNumberOfPresses,
                            static_cast<PressType>(pressTypeValue)));
        }
    }
...
PrintMaximumNumberOfAs::CountDetail PrintMaximumNumberOfAs::getMaxCountOfAOnScreenUsingNaiveRecursion(
        Count const remainingNumberOfPresses,
        PressType const pressType) const
{
        ...
        PressType const pressType) const
{
    ...
    {
        auto it = max_element(previousDetails.cbegin(), previousDetails.cend(), [](
                              CountDetail const& first, CountDetail const& second)
        {
                return first.numberInScreen < second.numberInScreen;
    });
...
{
    out << (int)countDetail.numberInScreen << ":"
        << (int)countDetail.numberInBuffer << ":"
