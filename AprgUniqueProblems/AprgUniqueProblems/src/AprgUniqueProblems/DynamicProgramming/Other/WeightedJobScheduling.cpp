...
auto endTimeJobComparator = [](
        WeightedJobScheduling::Job const& job1,
WeightedJobScheduling::Job const& job2) // sort by end time
{
return (job1.endTime != job2.endTime)
       ? job1.endTime < job2.endTime // sort on jobs that end early
       : job1.profit > job2.profit; // stylistic choice, put higher profits works
};

auto endTimeJobAndTimeComparator = [](
        WeightedJobScheduling::Job const& job,
WeightedJobScheduling::Time const time)
{
...
WeightedJobScheduling::WeightedJobScheduling(
        Jobs const& jobs)
    : m_jobs(jobs)
{}
...
WeightedJobScheduling::Profit WeightedJobScheduling::getMaxProfitByMonitoringTimeStampsUsingIterativeDP() const
{
    ...
    if(!m_jobs.empty())
    {
        ...
        {
            Index timeIndexBeforeJob = distance(timeStamps.begin(), find(timeStamps.begin(), timeStamps.end(), job.startTime));
            Index timeIndexAfterJob = distance(timeStamps.begin(), find(timeStamps.begin(), timeStamps.end(), job.endTime));
...
WeightedJobScheduling::Profit WeightedJobScheduling::getMaxProfitByMonitoringJobsUsingIterativeDP() const
{
    ...
    if(!m_jobs.empty())
    {
        ...
        for(Index jobIndex=1; jobIndex<jobsSortedByEndTime.size(); jobIndex++)
        {
            ...
            auto itLatestNonConflictingJob
                    = lower_bound(jobsSortedByEndTime.begin(), jobsSortedByEndTime.begin()+jobIndex, currentJob.startTime, endTimeJobAndTimeComparator);
