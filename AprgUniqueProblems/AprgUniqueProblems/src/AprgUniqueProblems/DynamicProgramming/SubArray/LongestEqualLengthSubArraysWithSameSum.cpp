...
LongestEqualLengthSubArraysWithSameSum::Index LongestEqualLengthSubArraysWithSameSum::getLongestTotalLengthOfSubArraysUsingPartialSums() const
{
    ...
    for(Index subArrayLength=m_values.size()/2; subArrayLength>0; subArrayLength--) // starts with highest length
    {
        for(Index startOfSubArray1=0; startOfSubArray1+(2*subArrayLength)<=m_values.size(); startOfSubArray1++)
        {
            ...
            Value sum2 = partialSumsQuery.getAccumulatedValueOnInterval(startOfSubArray2, startOfSubArray2+subArrayLength-1);
            if(sum1 == sum2)
            {
...
LongestEqualLengthSubArraysWithSameSum::Index LongestEqualLengthSubArraysWithSameSum::getLongestTotalLengthOfSubArraysByAccumulatingSum() const
{
    // Time Complexity: O(n^2)
    // Auxiliary Space: O(1)
