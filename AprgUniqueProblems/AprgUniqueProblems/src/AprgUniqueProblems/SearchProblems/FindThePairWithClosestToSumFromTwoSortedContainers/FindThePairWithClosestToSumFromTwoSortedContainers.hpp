...
class FindThePairWithClosestToSumFromTwoSortedContainers
{
public:
    using Value = typename Values::value_type;
    using ValuePair = std::pair<Value, Value>;

    FindThePairWithClosestToSumFromTwoSortedContainers() // values can be unsorted
    {}

    ValuePair getValuePairThatIsClosestToSum(
            Values const& values1,
            Values const& values2,
            Value const& sumToCheck)
    {
        ValuePair result{};
