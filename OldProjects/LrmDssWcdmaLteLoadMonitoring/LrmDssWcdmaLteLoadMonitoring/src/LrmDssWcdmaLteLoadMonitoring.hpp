...
typedef enum EDssWcdmaLoad
{
    EDssWcdmaLoad_Low     = 0,
    EDssWcdmaLoad_Medium  = 1,
    EDssWcdmaLoad_High    = 2
} EDssWcdmaLoad;
...
public:
    ...
    unsigned int calculateCellLoad(unsigned int const numberOfDchUsers, unsigned int const numberOfHsdpaUsers, bool const hasHsfachInTheCell) const;
    ...
    EDssWcdmaLoad determineLoadStateForTheNextFilterUpdateAndUpdateConsecutiveLowerLoadStateCountIfNeeded(EDssWcdmaLoad const currentLoadState, EDssWcdmaLoad const currentLoadStateOfTheAppliedFilter);

    void setConsecutiveLowerLoadStateCount(unsigned int const count);
    void setCommissioningLoadFactorValues(unsigned int const dchLoadFactor, unsigned int const hsdpaAndHsfachLoadFactor);
    void setCommissioningLoadThresholdValues(unsigned int const mediumLoadThreshold, unsigned int const highLoadThreshold);
