...
public:
    GenericMessage() : Message(MessageName::Empty) {}
    GenericMessage(MessageName const messageName, void* payloadBuffer, unsigned int const payloadSize)
        : Message(messageName), m_payloadBuffer(payloadBuffer, payloadSize) {}
    GenericMessage(
        MessageName const messageName, alba::AlbaMemoryBuffer const& buffer, ComponentName const sender,
        ComponentName const receiver)
        : Message(messageName, sender, receiver), m_payloadBuffer(buffer) {}
    GenericMessage(
        MessageName const messageName, void* payloadBuffer, unsigned int const payloadSize, ComponentName const sender,
        ComponentName const receiver)
