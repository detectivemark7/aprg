...
DelayedLinkIndices::PoolIdVector DelayedLinkIndices::getAvailablePoolIds(unsigned int const minimumIndices) const {
    ...
    for (PoolIdToNumberOfIndicesPair const& poolIdToNumberOfIndicesPair : m_poolIdToNumberOfIndices) {
        ...
        // Have available DLIs that is greater than or equal to the total amount of RX carriers from all cells in the
        // given PIC pool. See [Note 3].
...
void DelayedLinkIndices::prioritizeDliPoolForRel4(PoolIdVector& dliPools) const {
    // WBTS_CP_21702
    nth_element(
        dliPools.begin(), dliPools.begin(), dliPools.end(),
        [&](unsigned int const dliPoolId1, unsigned int const dliPoolId2) {
            bool result(false);
...
bool DelayedLinkIndices::hasConflictWithNextAvailablePool(
    PoolIdVector const& poolIds, unsigned int const minimumIndices) const {
    bool result(false);
    ...
    for (unsigned int const poolId : poolIds) {
        ...
        bool twoAndThreeConflict(
            minimumIndices > 8 &&
            ((poolId == 2 && availablePoolId == 3) || (poolId == 3 && availablePoolId == 2)));  // spec
...
LRM::LcgData::LcgData() : neededNumberOfDlis(0) {}

LRM::PowerGroupData::PowerGroupData() : numberOfK2sInPowerGroup(0) {}

LRM::LRM(ComponentName const componentName)
    : Component(componentName), m_masterTcomNid(FakeFrameworkHelpers::INVALID_DSP_ADDRESS) {}
...
TNumberOfItems LRM::getNumberOfK2sInFspForLcg(TAaSysComNid const fspAddress, TLocalCellGroupId const lcgId) const {
    ...
    for (DspAddressToDspDataPair const& dspAddressToDspDataPair : m_dspDatabase) {
        DspData const& dspData(dspAddressToDspDataPair.second);
        if (dspData.dspInfo.localCellGroupId == lcgId &&
            getFspAddressFromDspAddress(dspData.dspInfo.dspAddress) == fspAddress) {
            numberOfK2s++;
        }
    }
    return numberOfK2s;
...
TNumberOfItems LRM::getNumberOfPicAndNbicPacketsInFsp(TAaSysComNid const fspAddress) const {
    ...
    for (DspAddressToDspDataPair const& dspAddressToDspDataPair : m_dspDatabase) {
        DspData const& dspData(dspAddressToDspDataPair.second);
        if (getFspAddressFromDspAddress(dspData.dspInfo.dspAddress) == fspAddress &&
            ((dspData.mode == EDspMode_Kepler_MCD_CCDD && dspData.isNbic) ||
             dspData.mode == EDspMode_Kepler_DCD_PIC))  // spec?
        {
...
TNumberOfItems LRM::getNumberOfMcdDcdAndMcdCcdPacketsInFsp(TAaSysComNid const fspAddress) const {
    ...
    for (DspAddressToDspDataPair const& dspAddressToDspDataPair : m_dspDatabase) {
        DspData const& dspData(dspAddressToDspDataPair.second);
        if (getFspAddressFromDspAddress(dspData.dspInfo.dspAddress) == fspAddress &&
            (dspData.mode == EDspMode_Kepler_MCD_DCD || dspData.mode == EDspMode_Kepler_MCD_CCDD)) {
            numberOfK2PacketsWhichSatisfy++;
        }
    }
    return numberOfK2PacketsWhichSatisfy;
...
TNumberOfItems LRM::getNumberOfMcdCcdPacketsInFspForOtherLcgs(
    ...
    for (DspAddressToDspDataPair const& dspAddressToDspDataPair : m_dspDatabase) {
        ...
        if (dspData.dspInfo.localCellGroupId != lcgId &&
            getFspAddressFromDspAddress(dspData.dspInfo.dspAddress) == fspAddress &&
            dspData.mode == EDspMode_Kepler_MCD_CCDD) {
            numberOfMcdCcds++;
        }
    }
...
LRM::AddressesVector LRM::getDspAddressesForFspAddressAndLcgId(
...
void LRM::removeFspAddressInConflictWithDli(AddressesVector& fspAddresses, TLocalCellGroupId const lcgId) const {
    fspAddresses.erase(
        remove_if(
            fspAddresses.begin(), fspAddresses.end(),
            [&](TAaSysComNid const fspAddress) {
                ...
                return numberOfPicAndNbicPacketsInFsp >= 2 ||
                       hasConflictWithNextAvailablePool(fspAddress, poolIdsInFsp, lcgId);  // spec?
            }),
        fspAddresses.end());
}
...
void LRM::removeDspAddressesBasedOnFilterForSelectionMcdCcdD(AddressesVector& dspAddresses) const {
    ...
    dspAddresses.erase(
        remove_if(
            dspAddresses.begin(), dspAddresses.end(),
            [&](TAaSysComNid const dspAddress) {
                ...
                bool isIncorrectNumberOfK2InPowerGroup(
                    numberOfPowerGroupForDspAddress < 2);  // selection rule 1.  Belongs to Power Group where its two
                                                           // devices are allocated for the LCG.  See [Note 2].
                bool isAcceptableDspOriginalMode =
                    (dspMode == EDspMode_Kepler_DCD_DCD) || (dspMode == EDspMode_Kepler_DCD_PIC);  // WBTS_CP_19831
                // isTcomInThisK2() WBTS_CP_19862: Only devices where TCOM is not allocated in its ARM shall be
                // considered here.
...
void LRM::prioritizeDspAddressesForSelectionMcdCcdD(
    AddressesVector& dspAddresses, TLocalCellGroupId const lcgId) const {
    // WBTS_CP_19861
    nth_element(
        dspAddresses.begin(), dspAddresses.begin(), dspAddresses.end(),
        [&](TAaSysComNid const dspAddress1, TAaSysComNid const dspAddress2) {
            bool result(false);
            unsigned int numberOfMcdCcdPacketsInFspForDspAddress1(getNumberOfMcdCcdPacketsInFspForOtherLcgs(
                getFspAddressFromDspAddress(dspAddress1),
                lcgId));  // selection rule 2: Is located on baseband card with lowest number of MCD + CCD-d packets
                          // allocated for other LCGs.  See [Note 3].
            unsigned int numberOfMcdCcdPacketsInFspForDspAddress2(
                getNumberOfMcdCcdPacketsInFspForOtherLcgs(getFspAddressFromDspAddress(dspAddress2), lcgId));
            unsigned int numberOfK2InFspForDspAddress1(getNumberOfK2sInFspForLcg(
                getFspAddressFromDspAddress(dspAddress1),
                lcgId));  // selection rule 3: Is located on baseband card with largest amount of K2 assigned to LCG.
            unsigned int numberOfK2InFspForDspAddress2(
                getNumberOfK2sInFspForLcg(getFspAddressFromDspAddress(dspAddress2), lcgId));
...
void LRM::allocateMcdDcd(TLocalCellGroupId const lcgId) { logNoteOnComponent("LRM allocates MCD DCD."); }

void LRM::allocateCcdDcd(TLocalCellGroupId const lcgId) { logNoteOnComponent("LRM allocates CCD DCD."); }
...
void LRM::allocateMcdCcdDWithNbic(TLocalCellGroupId const lcgId) {
    ...
    if (nbicMcdCcdDAddress != FakeFrameworkHelpers::INVALID_DSP_ADDRESS) {
        unsigned int dliPoolId(
            getAvailableDliInAddress(getFspAddressFromDspAddress(nbicMcdCcdDAddress), lcgId));  // use save it
...
bool LRM::hasConflictWithNextAvailablePool(
    TAaSysComNid const fspAddress, DelayedLinkIndices::PoolIdVector const& poolIds,
...
void LRM::saveDliInfo(SLrmConfigurationDataInd const& lrmConfigurationData) {
    ...
    for (unsigned int index = 0; index < lrmConfigurationData.numberOfDelayedLinksInfo; index++) {
        ...
        m_coreToDli[getBoardFromSubrackAddress(currentLinksInfo.subrackAddress)].setIndices(
            currentLinksInfo.numberOfDelayedLinkIndices);
    }
}
