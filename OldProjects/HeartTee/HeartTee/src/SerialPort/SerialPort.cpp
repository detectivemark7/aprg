...
void SerialPort::set_blocking(int should_block) {
    ...
    if (tcsetattr(m_fd, TCSANOW, &tty) != 0) printf("error %d setting term attributes", errno);
}
...
void SerialPort::closeSerial() { close(m_fd); }
...
int SerialPort::getFd() { return m_fd; }

int SerialPort::getBytesRead() { return m_bytesRead; }
