...
void HeartTee::resetTime() { m_timeStamp = time(0); }
            ...
            if (m_process_state >= 5) {
                ...
                if (m_process_isStarted) {
                    if (m_process_currentEcg > 0x8000 || m_process_currentPulse > 0x8000) {
                        ...
                        cout << "m_process_currentEcg:" << m_process_currentEcg
                             << " m_process_currentPulse:" << m_process_currentPulse << " i:" << i << "\n";
                        exit(0);
                    }
...
void HeartTee::printDatabase() {
    for (HeartTeeData const& data : m_database) {
        ...
        cout << "second:" << ltm->tm_sec << " m_milliSecond:" << std::dec << data.m_milliSecond << " ecg:" << std::hex
             << data.m_ecg << " m_pulse:" << std::hex << data.m_pulse << "\n";
    }
...
void HeartTee::saveToSql() {
    ...
    if (m_database.size() > 0) {
        if (m_sqlConnect.getTableSize() <= SQLMAXTABLESIZE) {
            ...
            for (HeartTeeData const& database : m_database) {
                m_sqlConnect.write(
                    database.m_timeDate, database.m_milliSecond, database.m_pulse, database.m_ecg, pulsePerMin, 0,
                    maxValue - minValue);
            }
            m_database.clear();
