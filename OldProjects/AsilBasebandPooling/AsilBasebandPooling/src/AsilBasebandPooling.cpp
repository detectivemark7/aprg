...
AsilBasebandPooling::AsilBasebandPooling(LcgInitializerList const& lcgs, BasebandCardInitializerList const& basebandCards)
{
...
BasebandPoolingResult AsilBasebandPooling::performBasebandPoolingForAsil() const
{
    ...
    if(poolingResult.isSuccessful)
    {
        ...
        BasebandCardsSplitBasedOnNumberOfLcgs basebandCardsBasedOnNumberOfLcgs(getBasebandCardsSplitBetweenOneLcgAndMultipleLcgs(numberOfBasebandCardsWithMultipleLcgs));
        assignBasebandCardsWithOneLcg(poolingResult.keplerNidToLcgMap, lcgsInPriorityOrder, basebandCardsBasedOnNumberOfLcgs.basebandCardsWithOneLcg);
        assignBasebandCardsWithMultipleLcgs(poolingResult.keplerNidToLcgMap, lcgsInPriorityOrder, basebandCardsBasedOnNumberOfLcgs.basebandCardsWithMultipleLcgs);
    }
...
{
    stable_sort(basebandCardsWithPrioritization.begin(), basebandCardsWithPrioritization.end(), [](
                BasebandCard const& basebandCard1,
                BasebandCard const& basebandCard2)
    {
...
{
    stable_sort(lcgsInPriorityOrder.begin(), lcgsInPriorityOrder.end(), [](
                Lcg const& lcg1,
                Lcg const& lcg2)
    {
        ...
        SetOfBasebandCards const& basebandCardsWithOneLcg) const
{
    ...
    copy(basebandCardsWithOneLcg.cbegin(), basebandCardsWithOneLcg.cend(), back_inserter(basebandCardsWithPrioritization));
...
{
    VectorOfBasebandCards basebandCardsWithPrioritization;
    copy(basebandCardsWithMultipleLcgs.cbegin(), basebandCardsWithMultipleLcgs.cend(), back_inserter(basebandCardsWithPrioritization));
    sortAndPrioritizeBasebandCards(basebandCardsWithPrioritization);
