...
public:
    void handle() override { std::cout << "State B handled.\n"; }
    // ...
};
...
// -----> Because all state-specific code lives in a State subclass, new states and transitions can be added easily by
...
// -----> Adding a new state could require changing several operations, which complicates maintenance (violates the
// open/close priniciple).
// -------> The state pattern avoids this problem but might introduce another, it distributes behavior of different
// states across several State subclasses.
...
// ---> State object can protect the Context from inconsistent internal states because state transistion are atomic from
// Context perspective.
...
// -----> However, it is generally more flexible, to let the State subclasses themselves specify their successor state
// and when to make the transistion.
// -------> This requires adding an interface to the Context that lets State objects set the Context current state
// explicitly.
// ---> Decentralizing the transition logic in this way makes it easy to modify or extend the logic by defining new
// state subclasses.
// -----> A disadvantage of decentralization is that one State subclass will have knowledge of at least one other, which
// introduces implementation dependencies.
...
// -----> In effect, this approach converts conditional code (states as variable) or virtual dispatch (state pattern)
// into a table look-up.
// ---> Advantage: You can change the transition criteria by modifying data instead of changing program code.
// ---> Disadvantages:
// -----> A table look-up is often less efficient that a virtual function call (Note: Can't claim really without
// testing).
// -----> Putting transition logic into a uniform, tabular format makes the transition criteria less explicit and
// therefore harder to understand.
// -----> Its usually difficult to add actions to accompany the state transitions.
// -------> The table-driven approach captures the states and their transitions, but it must be augmented to perform
// arbitiary computation on each transition.
...
// -------> This is preferable if the states that will be entered aren't known at runtime and contexts change state
// infrequently.
// -------> This approach avoids creating objects that wont be used, which is important if the State objects store a lot
// of information.
// -----> (2) to create State objects ahead of time and never destroying them
// -------> This is better when the states changes occur rapidly, in which case you want to avoid destroying state to
// avoid delays.
// -------> Instantiation cost are paid once up-front and destruction costs only occurs when program is terminated.
// -------> This approach might be inconvenient, though, because the Context must keep references to all states that
// might be entered.
