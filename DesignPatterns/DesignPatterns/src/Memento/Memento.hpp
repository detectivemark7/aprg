...
// ---> The pattern shields other from potentially complex Originator internals, thereby preserving encapsulation
// boundaries.
// -> It simplifies Originator.
// ---> In other encapsulation-preserving designs, Originator keeps the version of internal state that clients have
// requested.
// ---> That puts all the storage managment burden on Originator.
// ---> Having clients manage the state they ask for simplifies Originator and keeps clients from hacing to notify
// orgiginator when they're done.
// -> Using mementos might be expensive.
// ---> Mementos might incur considerable overhead if Originator must copy large amounts of information to store in the
// memento
// -----> or if clients create and return mementos to originator often enough.
// ---> Unless encapsulating and restoring Originator state is cheap, the pattern might not be appropriate (see
// incremental part in Implementation).
...
// ---> For example, undoable commands in a history list can use mementos to ensure that commands are restored to their
// state when they undone.
