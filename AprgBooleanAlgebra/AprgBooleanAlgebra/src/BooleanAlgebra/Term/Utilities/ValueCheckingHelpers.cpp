...
bool willHaveNoEffectOnOrOperation(Term const& term) { return term.isEmpty() || isTheValue(term, false); }
...
bool isTheValue(Constant const& constant, bool const value) { return constant.getBooleanValue() == value; }
...
bool isTheValue(Expression const& expression, bool const value) {
    ...
    if (wrappedTerms.size() == 1) {
        result = isTheValue(getTermConstReferenceFromUniquePointer(wrappedTerms.front().baseTermPointer), value);
    }
    return result;
}
