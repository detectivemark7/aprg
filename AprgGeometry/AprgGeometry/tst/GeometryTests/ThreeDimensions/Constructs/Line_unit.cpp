...
TEST(ThreeDimensionsLineTest, HorizontalLine) {
    ...
    EXPECT_FALSE(line.calculateXFromY(1));
    ASSERT_TRUE(line.calculateXFromZ(1));
    EXPECT_EQ(-0.5, line.calculateXFromZ(1).value());
    ASSERT_TRUE(line.calculateYFromX(1));
    EXPECT_EQ(3, line.calculateYFromX(1).value());
    ASSERT_TRUE(line.calculateYFromZ(1));
    EXPECT_EQ(3, line.calculateYFromZ(1).value());
    ASSERT_TRUE(line.calculateZFromX(1));
    EXPECT_EQ(-2, line.calculateZFromX(1).value());
    EXPECT_FALSE(line.calculateZFromY(1));
}
...
TEST(ThreeDimensionsLineTest, VerticalLine) {
    ...
    ASSERT_TRUE(line.calculateXFromY(1));
    EXPECT_EQ(2, line.calculateXFromY(1).value());
    ASSERT_TRUE(line.calculateXFromZ(1));
    EXPECT_EQ(2, line.calculateXFromZ(1).value());
    EXPECT_FALSE(line.calculateYFromX(1));
    ASSERT_TRUE(line.calculateYFromZ(1));
    EXPECT_EQ(-0.5, line.calculateYFromZ(1).value());
    EXPECT_FALSE(line.calculateZFromX(1));
    ASSERT_TRUE(line.calculateZFromY(1));
    EXPECT_EQ(-2, line.calculateZFromY(1).value());
}
...
TEST(ThreeDimensionsLineTest, ParallelWithZAxis) {
    ...
    EXPECT_EQ(6, line.getZInitialValue());
    ASSERT_TRUE(line.calculateXFromY(1));
    EXPECT_EQ(-0.5, line.calculateXFromY(1).value());
    EXPECT_FALSE(line.calculateXFromZ(1));
    ASSERT_TRUE(line.calculateYFromX(1));
    EXPECT_EQ(-2, line.calculateYFromX(1).value());
    EXPECT_FALSE(line.calculateYFromZ(1));
    ASSERT_TRUE(line.calculateZFromX(1));
    EXPECT_EQ(6, line.calculateZFromX(1).value());
    ASSERT_TRUE(line.calculateZFromY(1));
    EXPECT_EQ(6, line.calculateZFromY(1).value());
}
...
TEST(ThreeDimensionsLineTest, LineNotParallelWithXYZAxis) {
    ...
    EXPECT_EQ(-6, line.getZInitialValue());
    ASSERT_TRUE(line.calculateXFromY(1));
    EXPECT_EQ(5, line.calculateXFromY(1).value());
    ASSERT_TRUE(line.calculateXFromZ(1));
    EXPECT_EQ(4, line.calculateXFromZ(1).value());
    ASSERT_TRUE(line.calculateYFromX(1));
    EXPECT_EQ(5, line.calculateYFromX(1).value());
    ASSERT_TRUE(line.calculateYFromZ(1));
    EXPECT_EQ(2, line.calculateYFromZ(1).value());
    ASSERT_TRUE(line.calculateZFromX(1));
    EXPECT_EQ(7, line.calculateZFromX(1).value());
    ASSERT_TRUE(line.calculateZFromY(1));
    EXPECT_EQ(-1, line.calculateZFromY(1).value());
}
...
TEST(ThreeDimensionsLineTest, LineWithSlopeSet) {
    ...
    EXPECT_EQ(-0.5, line.calculateXFromY(1).value());
    ASSERT_TRUE(line.calculateXFromZ(1));
    EXPECT_EQ(-5, line.calculateXFromZ(1).value());
    ASSERT_TRUE(line.calculateYFromX(1));
    EXPECT_EQ(2, line.calculateYFromX(1).value());
    ASSERT_TRUE(line.calculateYFromZ(1));
    EXPECT_EQ(-2, line.calculateYFromZ(1).value());
    ASSERT_TRUE(line.calculateZFromX(1));
    EXPECT_EQ(3, line.calculateZFromX(1).value());
    ASSERT_TRUE(line.calculateZFromY(1));
    EXPECT_EQ(2.5, line.calculateZFromY(1).value());
}
...
TEST(ThreeDimensionsLineTest, LineWithSumOfCoefficientsEqualToZero) {
    ...
    EXPECT_EQ(0, line.getZInitialValue());
    ASSERT_TRUE(line.calculateXFromY(1));
    EXPECT_EQ(-1, line.calculateXFromY(1).value());
    ASSERT_TRUE(line.calculateXFromZ(1));
    EXPECT_EQ(-1, line.calculateXFromZ(1).value());
    ASSERT_TRUE(line.calculateYFromX(1));
    EXPECT_EQ(0, line.calculateYFromX(1).value());
    ASSERT_TRUE(line.calculateYFromZ(1));
    EXPECT_EQ(1, line.calculateYFromZ(1).value());
    ASSERT_TRUE(line.calculateZFromX(1));
    EXPECT_EQ(0, line.calculateZFromX(1).value());
    ASSERT_TRUE(line.calculateZFromY(1));
    EXPECT_EQ(1, line.calculateZFromY(1).value());
}
