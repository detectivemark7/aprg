...
bool EllipticHyperboloid::operator==(EllipticHyperboloid const& ellipticHyperboloid) const {
    return (m_center == ellipticHyperboloid.m_center) && isAlmostEqual(m_aValue, ellipticHyperboloid.m_aValue) &&
...
double EllipticHyperboloid::calculateYFromXAndZ(double const x, double const z, double const signOfRoot) const {
    return pow(m_oneWithSign - pow((x - m_center.getX()) / m_aValue, 2) + pow((z - m_center.getZ()) / m_cValue, 2),
               0.5) *
               signOfRoot * m_bValue +
           m_center.getY();
}
...
double EllipticHyperboloid::calculateZFromXAndY(double const x, double const y, double const signOfRoot) const {
    return pow(m_oneWithSign - pow((x - m_center.getX()) / m_aValue, 2) - pow((y - m_center.getY()) / m_bValue, 2),
               0.5) *
               signOfRoot * m_cValue +
           m_center.getZ();
}
...
ostream& operator<<(ostream& out, EllipticHyperboloid const& ellipticHyperboloid) {
    out << "(center: " << ellipticHyperboloid.m_center << " a: " << ellipticHyperboloid.m_aValue
        << " b: " << ellipticHyperboloid.m_bValue << " c: " << ellipticHyperboloid.m_cValue << ")";
    return out;
}
