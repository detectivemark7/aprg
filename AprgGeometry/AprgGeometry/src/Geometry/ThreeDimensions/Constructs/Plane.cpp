...
Plane::Plane(double const aCoefficient, double const bCoefficient, double const cCoefficient, Point const& pointInPlane)
    : m_aCoefficient(aCoefficient), m_bCoefficient(bCoefficient), m_cCoefficient(cCoefficient), m_dCoefficient(0) {
    calculateDCoefficientUsingCoefficientsABCAndAPoint(pointInPlane);
}

Plane::Plane(double const aCoefficient, double const bCoefficient, double const cCoefficient, double const dCoefficient)
...
bool Plane::operator==(Plane const& plane) const {
    return isAlmostEqual(m_aCoefficient, plane.m_aCoefficient) && isAlmostEqual(m_bCoefficient, plane.m_bCoefficient) &&
           isAlmostEqual(m_cCoefficient, plane.m_cCoefficient) && isAlmostEqual(m_dCoefficient, plane.m_dCoefficient);
}
...
void Plane::calculateDCoefficientUsingCoefficientsABCAndAPoint(Point const& first) {
    m_dCoefficient =
        -(m_aCoefficient * first.getX()) - (m_bCoefficient * first.getY()) - (m_cCoefficient * first.getZ());
}

ostream& operator<<(ostream& out, Plane const& plane) {
    out << plane.m_aCoefficient << "*x + " << plane.m_bCoefficient << "*y + " << plane.m_cCoefficient << "*z + "
        << plane.m_dCoefficient << " = 0";
    return out;
}
