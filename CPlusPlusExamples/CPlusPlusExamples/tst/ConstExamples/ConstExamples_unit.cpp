...
void function3(Foo const*)
{
    cout << "function3 with const*\n";
}
...
TEST(ConstExamplesTest, TestFunctionMatchingWithConstInFunctionSignatureOfAClass)
{
    ...
    // Note: If the member function is const, then you can use it in non const structures to add "const"ness to the operation.
}
...
TEST(ConstExamplesTest, TestMutabilityOfMutableMembersInAClass)
{
    ...
    public:
        ...
        int getNumberOfTimesCalled() const
        {
            return m_numberOfTimesCalled;
        }
    private:
    ...
    cout << "data: [" << dataHolder.getCheckSum() << "] number of times called: [" << dataHolder.getNumberOfTimesCalled() << "]\n";
    ...
    // ---> If two or more thread access the same memory location without synchronization, and at least one is a writer, you have undefined behavior.

    // Note: "mutable" member variables are useful, but can be a nuisance because it breaks const (especially in multithreaded applications)
}
...
TEST(ConstExamplesTest, TestOptimzationOfConstParameters)
{
    ...
    // -----> const is still principally uses as a tool that lets human class designer better implement handcrafted optimizations
    ...
    // Note: "const"ness generally does not optimize for faster code (except of constexpr because its compiled beforehand).
