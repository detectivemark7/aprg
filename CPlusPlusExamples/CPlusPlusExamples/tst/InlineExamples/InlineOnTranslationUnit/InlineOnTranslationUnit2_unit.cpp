...
//int nonInlineAtTranslationUnit=500; // Linker error: multiple definition of `alba::InlineOnTranslationUnits::nonInlineAtTranslationUnit'
inline int externInlineInteger=620; // definition (complete type), different definition (results in undefined behavior)
...
TEST(InlineOnTranslationUnit2Test, DISABLED_VariableValuesAreCorrect) // Flaky test
{
    ...
    EXPECT_EQ(410, inlineIntegerAtTranslationUnit); // Undefined behavior really but in this case in takes the implementation from TranslationUnit1
    //EXPECT_EQ(0, nonInlineAtTranslationUnit); // no "nonInlineAtTranslationUnit" here because it conflicts with TranslationUnit1
...
TEST(InlineOnTranslationUnit2Test, VariableValuesAreChangedAndReflectedOnOtherTranslationUnit)
{
    ...
    //nonInlineAtTranslationUnit = 503; // no "nonInlineAtTranslationUnit" here because it conflicts with TranslationUnit1
...
TEST(InlineOnTranslationUnit2Test, FunctionReturnValuesAreCorrect)
{
    ...
    //EXPECT_EQ(1, inlineFreeFunction()); // Undefined behavior really but in this case in takes the implementation from TranslationUnit1
}
