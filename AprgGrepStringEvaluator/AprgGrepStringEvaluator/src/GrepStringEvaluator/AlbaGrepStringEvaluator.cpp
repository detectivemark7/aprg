...
AlbaGrepStringEvaluator::AlbaGrepStringEvaluator(string const& condition)
    : m_isEvaluatorInvalid(false), m_errorMessage() {
...
void AlbaGrepStringEvaluator::extractTokens(string const& condition) {
    ...
    m_tokens.erase(
        remove_if(
            m_tokens.begin(), m_tokens.end(), [](AlbaGrepStringToken const& token) { return token.isToBeIgnored(); }),
        m_tokens.end());
...
void AlbaGrepStringEvaluator::extractTokensWhileOnString(
    bool& isOnString, string& stringToBuild, char const& currentCharacter) {
    if (']' == currentCharacter) {
...
void AlbaGrepStringEvaluator::extractTokensWhileNotOnString(
    bool& isOnString, char const& currentCharacter, int& parenthesisCount) {
    if ('[' == currentCharacter) {
        isOnString = true;
    ...
    for (AlbaGrepStringToken const& token : m_tokens) {
        if (token.isOperator()) {
            if (token.isPrefixOperation()) {
                infixEvaluator.addTerm(InfixEvaluator::Term(
                    token.getOperatorType(), EvaluatorTerm::OperatorSyntaxType::PrefixUnary,
                    token.getOperatorPriority()));
            } else if (token.isBiDirectionalOperation()) {
                infixEvaluator.addTerm(InfixEvaluator::Term(
                    token.getOperatorType(), EvaluatorTerm::OperatorSyntaxType::Binary, token.getOperatorPriority()));
            } else if (token.isOpeningParenthesis()) {
                infixEvaluator.addTerm(InfixEvaluator::Term(
                    token.getOperatorType(), EvaluatorTerm::OperatorSyntaxType::StartGroup,
                    token.getOperatorPriority()));
            } else if (token.isClosingParenthesis()) {
                infixEvaluator.addTerm(InfixEvaluator::Term(
                    token.getOperatorType(), EvaluatorTerm::OperatorSyntaxType::EndGroup, token.getOperatorPriority()));
            }
        ...
        case AlbaGrepStringOperatorType::NotOperator:
            switch (currentCharacter) {
                case '^':
                    ...
                    break;
                case '!':
                    previousToken.setOperatorType(AlbaGrepStringOperatorType::IgnoreOperator);
                    break;
                default:
                    invalidOperator = true;
                    break;
        ...
        default:
            switch (currentCharacter) {
                case '!':
                    ...
                    break;
                case '&':
                    m_tokens.emplace_back(AlbaGrepStringOperatorType::AndOperator, string("") + currentCharacter);
                    break;
                case '|':
                    m_tokens.emplace_back(AlbaGrepStringOperatorType::OrOperator, string("") + currentCharacter);
                    break;
                case '^':
                    m_tokens.emplace_back(AlbaGrepStringOperatorType::XorOperator, string("") + currentCharacter);
                    break;
                default:
                    break;
            }
