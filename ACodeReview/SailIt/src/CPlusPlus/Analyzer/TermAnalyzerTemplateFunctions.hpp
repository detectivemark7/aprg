...
bool TermAnalyzer::isSingleLineComparisonSatisfiedThenMoveLooper(
    Looper& movableLooper, Container const& expectedTerms) {
...
bool TermAnalyzer::isTheMismatchWithWhiteSpaceAndMoveLooper(
    Looper& movableLooper, ExpectedTermsIteratorType& itExpected) {
...
template <typename ExpectedTermsIteratorType>
bool TermAnalyzer::isTheMismatchWithIgnorableTermAndMoveLooper(
    Looper& movableLooper, ExpectedTermsIteratorType& itExpected) {
...
bool TermAnalyzer::isBothBracketsFoundAndMoveLoopers(
...
template <LooperConnector looperConnector>
bool TermAnalyzer::isBothParenthesesFoundAndMoveLoopers(
...
template <LooperConnector looperConnector>
bool TermAnalyzer::isBothAngleBracketsFoundAndMoveLoopers(
    Looper const& startLooper, Looper& afterOpeningAngleBracket, Looper& afterClosingAngleBracket) {
    TemporaryFindings temporaryFindings(m_findingsBuffer);
    Looper afterNewLineAndWhiteSpaces(startLooper);
    connectLooper<looperConnector>(afterNewLineAndWhiteSpaces);
    if (afterNewLineAndWhiteSpaces.isNotFinished() &&
...
template <LooperConnector looperConnector>
bool TermAnalyzer::isBothBracesFoundAndMoveLoopers(
...
template <LooperConnector looperConnector>
bool TermAnalyzer::isBothBracesFoundWithSemicolonAndMoveLoopers(
...
template <LooperConnector looperConnector>
bool TermAnalyzer::isSemiColonFoundAndMoveLooper(Looper& afterSemiColon) {
    TemporaryFindings temporaryFindings(m_findingsBuffer);
...
VectorOfTerms TermAnalyzer::getTerms(Looper const& startLooper, Container const& expectedTerms) {
    ...
    while (updateLooper.isNotFinished() && termIterator != expectedTerms.end())  // copy if anyone?
    {
        ...
        if (*termIterator == termInLooper) {
            ...
            CheckerHelpers::isIgnorable(termInExpectedTerms) || CheckerHelpers::isWhiteSpace(termInExpectedTerms) ||
            CheckerHelpers::isNewLine(termInExpectedTerms)) {
...
void TermAnalyzer::incrementLooperIfWhiteSpaceForMacro(Looper& movableLooper) {
...
template <FindingsToAdd findingsToAdd>
void TermAnalyzer::incrementLooperIfWhiteSpaceAndOneNewLine(Looper& movableLooper) {
    while (movableLooper.isNotFinished()) {
        if (CheckerHelpers::isWhiteSpace(movableLooper.getContentReference())) {
            addFindingsForUnexpectedWhiteSpaceIfNeeded<findingsToAdd>(movableLooper);
...
template <FindingsToAdd findingsToAdd>
void TermAnalyzer::incrementLooperIfWhiteSpaceAndNewLine(Looper& movableLooper) {
    while (movableLooper.isNotFinished()) {
...
template <FindingsToAdd findingsToAdd>
void TermAnalyzer::decrementLooperIfWhiteSpaceAndNewLine(Looper& movableLooper) {
