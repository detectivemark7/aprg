...
bool TermAnalyzer::isModifiedDueToIncludeMacroWithAngleBrackets(Looper const& startLooper) {
    ...
    if (isSingleLineComparisonSatisfiedThenMoveLooper<LooperConnector::None>(compareLooper, expectedTerms)) {
        ...
        if (isBothAngleBracketsFoundAndMoveLoopers<LooperConnector::WhiteSpaceAndNewLine>(
                compareLooper, afterOpeningAngleBracket, afterClosingAngleBracket)) {
            string includeFileName(
                accumulateLooperContentsToString(Looper(afterOpeningAngleBracket, afterClosingAngleBracket - 1)));
            incrementLooperIfWhiteSpaceAndOneNewLine<FindingsToAdd::ExpectsNewLineAndUnexpectsWhiteSpace>(
                afterClosingAngleBracket);
...
bool TermAnalyzer::isModifiedDueToDefineMacroWithParameters(
    ...
    if (isBothParenthesesFoundAndMoveLoopers<LooperConnector::WhiteSpaceAndNewLine>(
            afterIdentifierLooper, afterOpeningParenthesis, afterClosingParenthesis)) {
            ...
            if (hasTermsForMacro(macroEquivalentLooper)) {
                if (isStringFoundInLooper(macroEquivalentLooper, macroName)) {
                    addFinding(
                        ...
                        "Cannot add macro:" + macroName + ". Macro name is used in macro equivalent.", startLooper);
                } else {
...
bool TermAnalyzer::isModifiedDueToDefineMacroWithNoParameters(
    Looper const& startLooper, Looper const& afterIdentifierLooper, string const& macroName) {
...
void TermAnalyzer::copyDatabaseOfIncludeFile(Looper const& startLooper, string const& includeFileName) {
    ...
    if (m_optionalSailIt) {
        ...
        if (fileDirectoryDatabase.isFileIncluded(m_baseDirectory, includeFileName)) {
            ...
            addFinding(
                constructFileLocator(__FILE__, __LINE__), "Include file:" + includeFileName + " is not found.",
                startLooper);
        }
    }
}
...
void TermAnalyzer::checkMacroParameters(Looper const& startLooper, CPlusPlusMacro& cPlusPlusMacro) {
    ...
    while (parameterLooper.isNotFinished()) {
        ...
        if (isMultiLineComparisonSatisfiedThenMoveLooper<LooperConnector::WhiteSpaceAndNewLine>(
                parameterLooper, nextArgumentForm)) {
            macroParameters.push_back(firstTermLooper.getContentReference().getString());
                       ...
                       parameterLooper, lastArgumentForm)) {
            macroParameters.push_back(firstTermLooper.getContentReference().getString());
            break;
        } else if (isMultiLineComparisonSatisfiedThenMoveLooper<LooperConnector::WhiteSpaceAndNewLine>(
                       parameterLooper, variadicArgumentForm)) {
            macroParameters.push_back("__VA_ARGS__");
            cPlusPlusMacro.setVariadicParameterIndex(macroParameters.size() - 1);
            break;
        } else {
            addFinding(
                constructFileLocator(__FILE__, __LINE__), "Unprocessed term in macro parameters", parameterLooper);
            break;
        }
    }
