...
bool TermTranslator::isFileValid() { return m_isFileValid; }
...
bool TermTranslator::hasWhiteSpace(string& lineString, int& index) {
    ...
    if (isWhiteSpace(lineString[index])) {
        ...
        if (" " != whiteSpaceString) {
            string locator = lineString.substr(0, start) + "[" + lineString.substr(start, index - start) + "]" +
                             lineString.substr(index);
            m_findings.addFinding(
                m_lineNumber, constructFileLocator(__FILE__, __LINE__), "Extra white space found", locator);
        }
        return true;
    }
    return false;
...
bool TermTranslator::isKeyword(string const& word) {
    return any_of(
        m_stringOfKeywords.begin(), m_stringOfKeywords.end(), [word](string& keyword) { return keyword == word; });
}

bool TermTranslator::isCPlusPlusIgnorableIdentifier(string const& word) {
    return any_of(
        m_stringOfIgnorableIdentifiers.begin(), m_stringOfIgnorableIdentifiers.end(),
        [word](string& keyword) { return keyword == word; });
}
