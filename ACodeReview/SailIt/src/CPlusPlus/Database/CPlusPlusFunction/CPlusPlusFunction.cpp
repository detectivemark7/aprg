...
bool CPlusPlusFunction::isFunctionSignatureExistBasedFromFunctionCall(
...
bool CPlusPlusFunction::isFunctionSignatureExistAndNamesDoesNotMatch(
    CPlusPlusFunctionSignature const& cPlusPlusFunctionSignature) const {
    // stl algorithm any_of
    for (CPlusPlusFunctionSignature const& signature : m_signatures) {
...
void CPlusPlusFunction::replaceTemplateParameters(
    ...
    for (CPlusPlusFunctionSignature& signature : m_signatures) {
        for (CPlusPlusFunctionParameter& parameter : signature.getFunctionParametersReference()) {
            CPlusPlusClassTemplateParameters instantiationParameters =
                templateInstantiation.getTemplateParametersReference();
            auto it = instantiationParameters.cbegin();
            for (CPlusPlusClassTemplateParameter const& templateParameter :
                 templateSignature.getTemplateParametersReference()) {
                if (templateParameter.getType() == parameter.getType() &&
                    templateParameter.getName() == parameter.getName()) {
        ...
        for (CPlusPlusClassTemplateParameter const& templateParameter :
             templateSignature.getTemplateParametersReference()) {
            if (templateParameter.getType() == signature.getReturnType() &&
                templateParameter.getType().isTemplateType()) {
...
CPlusPlusFunctionSignatures& CPlusPlusFunction::getFunctionSignaturesReference() { return m_signatures; }

CPlusPlusType CPlusPlusFunction::getReturnTypeWithSignature(
    CPlusPlusFunctionSignature const& cPlusPlusFunctionSignature) const {
    for (CPlusPlusFunctionSignature const& signature : m_signatures) {
        if (signature.isEqualBasedFromFunctionCall(cPlusPlusFunctionSignature)) {
