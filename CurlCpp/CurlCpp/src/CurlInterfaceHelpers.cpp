...
static int xferinfo(void*, curl_off_t dlTotal, curl_off_t dlNow, curl_off_t ulTotal, curl_off_t ulNow) {
    ...
    cout << "\rProgress: " << percentProgress << "% (DownLink: " << dlNow << " of " << dlTotal << "   UpLink: " << ulNow
         << " of " << ulTotal << ")" << std::flush;
    return 0;
}
...
void CurlInterface::addToCurlEasy<DownloadType::MozillaFireFox>(curl_easy& easy) {
    ...
    chunk = curl_slist_append(
        chunk, "User-Agent: Mozilla/5.0 (Windows NT 6.1; WOW64; rv:39.0) Gecko/20100101 Firefox/39.0");
...
void CurlInterface::createOutputStream(
    unique_ptr<ofstream>& outputStream, OutputFileType outputFileType, string const& fileLocation) {
...
bool CurlInterface::download(string url, ofstream& outputFile, function<void(curl_easy&)> additionalConfig) {
    ...
    curl_ios<ostream> ios(outputFile);
    curl_easy easy(ios);
...
bool CurlInterface::downloadFile(
    string const& url, string const& fileLocation, OutputFileType outputFileType,
...
bool CurlInterface::downloadFileUntilSuccessful(
    string const& url, string const& fileLocation, OutputFileType outputFileType,
    CurlInterface::ConfigurationFunction additionalConfig) {
...
bool CurlInterface::downloadFileWithFiniteNumberOfTries(
    string const& url, string const& fileLocation, OutputFileType outputFileType, int const totalNumberOfTries,
    CurlInterface::ConfigurationFunction additionalConfig) {
    bool isSuccessful(false);
    for (int numberOfTries = 1; (!isSuccessful) && (numberOfTries <= totalNumberOfTries); numberOfTries++) {
        ...
        isSuccessful = download(
            url, *(outputFile.get()), [numberOfTries, additionalConfig](curl_easy& easy) { additionalConfig(easy); });
        if (!isSuccessful) {
            cout << "   --> Download failed and retrying in a few seconds. \nFile: [" << fileLocation << "] \nFrom: ["
                 << url << "]" << endl;
            Sleep(5000);
