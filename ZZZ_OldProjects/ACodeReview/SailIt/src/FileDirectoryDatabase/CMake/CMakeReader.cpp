...
bool CMakeReader::isFileValid() { return m_isFileValid; }

VariableMapType& CMakeReader::getVariableMapReference() { return m_variableMap; }
...
void CMakeReader::addVariable(string const& variableName, string const& contents) {
    m_variableMap[variableName].emplace(getStringWithoutRedundantWhiteSpace(
        getCorrectPathWithoutDoublePeriod<'\\'>(getCorrectPathWithReplacedSlashCharacters<'\\'>(contents))));
}

void CMakeReader::clearVariable(string const& variableName) { m_variableMap[variableName].clear(); }
...
void CMakeReader::readFile() {
    if (m_isFileValid) {
        ...
        while (m_albaFileReader.isNotFinished() || !lineString.empty()) {
            ...
            if (isIgnorable(lineString)) {
                lineString = string("");
                continue;
            }

            string firstCommand = getStringWithCapitalLetters(getNextCMakeIdentifier(lineString, index));
            if (firstCommand == "PROJECT") {
                processProjectCommand(lineString, index);
            } else if (firstCommand == "SET") {
                processSetCommand(lineString, index);
            } else if (firstCommand == "INCLUDE_DIRECTORIES") {
                processIncludeDirectoriesCommand(lineString, index);
            } else if (firstCommand == "INCLUDE") {
                processIncludeCommand(lineString, index);
            } else if (firstCommand == "ADD_SUBDIRECTORY") {
                processAddSubDirectoryCommand(lineString, index);
            } else if (firstCommand == "ADD_EXECUTABLE") {
                includeSecondArgumentToFilesAndDirectories(lineString, index);
            } else if (firstCommand == "ADD_LIBRARY") {
                includeSecondArgumentToFilesAndDirectories(lineString, index);
            } else {
                ignoreEntireLine(lineString, index);
            }  // CMAKE does not have line separators, its hard to assume
...
void CMakeReader::processSetCommand(string& lineString, int& index) {
    ...
    replaceVariableWithRealValuesInStringAndDoOperation(
        m_variableMap.begin(), contents, [&](string stringWithRealValues) {
            tempVariableMap[variableName].emplace(
                getStringWithoutQuotations(getStringWithoutRedundantWhiteSpace(stringWithRealValues)));
        });
...
void CMakeReader::processIncludeDirectoriesCommand(string& lineString, int& index) {
    ...
    index = 0;
    string contents(extractContentsUntilCloseParenthesis(lineString, index));
    // cout << "CMakeReader::processIncludeDirectoriesCommand File:"<<m_fullPathOfFile<<"
    // lineString:["<<lineString<<"]\n";
    replaceVariableWithRealValuesInStringAndDoOperation(
        m_variableMap.begin(), contents,
        [&](string stringWithRealValues) { addToFilesAndDirectoriesDatabase(stringWithRealValues); });
}
        ...
        m_variableMap.begin(), contents, [&](string stringWithRealValues) {
            if (!hasCMakeVariables(stringWithRealValues)) {
                separateStringsUsingWhiteSpaceAndDoOperation(stringWithRealValues, [&](string stringWithoutWhiteSpace) {
                    addCMakeDirectoryIfNeededAndDoOperation(
                        stringWithoutWhiteSpace,
                        [&](string stringWithCMakeDirectory) { listOfIncludeFiles.emplace(stringWithCMakeDirectory); });
                });
            }
        });
    ...
    replaceVariableWithRealValuesInStringAndDoOperation(
        m_variableMap.begin(), contents, [&](string stringWithRealValues) {
            if (!hasCMakeVariables(stringWithRealValues)) {
                separateStringsUsingWhiteSpaceAndDoOperation(stringWithRealValues, [&](string stringWithoutWhiteSpace) {
                    addCMakeDirectoryIfNeededAndDoOperation(
                        stringWithoutWhiteSpace, [&](string stringWithCMakeDirectory) {
...
void CMakeReader::includeSecondArgumentToFilesAndDirectories(string& lineString, int& index) {
    ...
    index = 0;
    string contents(extractContentsUntilCloseParenthesis(lineString, index));
    // cout << "CMakeReader::includeSecondArgumentToFilesAndDirectories: File:"<<m_fullPathOfFile<<"
    // lineString:["<<lineString<<"]\n";
    replaceVariableWithRealValuesInStringAndDoOperation(
        m_variableMap.begin(), contents,
        [&](string stringWithRealValues) { addToFilesAndDirectoriesDatabase(stringWithRealValues); });
...
void CMakeReader::separateStringsUsingWhiteSpaceAndDoOperation(
    ...
    if (!longString.empty()) {
        operationForEachString(longString);
    }
}
...
void CMakeReader::ignoreEntireLine(string const& string1, int& index) { index = string1.length(); }
