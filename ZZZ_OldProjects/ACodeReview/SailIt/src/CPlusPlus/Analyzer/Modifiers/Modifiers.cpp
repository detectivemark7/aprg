...
bool TermAnalyzer::isModifiedDueToUpdatedIdentifiers(
    Looper const& startLooper, LooperIndex& previousUpdatedIndexForMacro, LooperIndex& previousUpdatedIndexForOthers) {
...
void TermAnalyzer::combineToASingleTermWithValueType(
...
void TermAnalyzer::replaceLooperContentsWithASingleTerm(
    Looper const& startLooper, Looper const& endLooper, Term const& term) {
    Looper combineLooper(startLooper, endLooper);
    Term& firstTerm = startLooper.getContentReference();
    firstTerm = term;
    combineLooper++;
...
void TermAnalyzer::replaceTermWithMacroEquivalent(Looper const& replaceLooper, CPlusPlusMacro& cPlusPlusMacro) {
    ...
    if (isBothParenthesesFoundAndMoveLoopers<LooperConnector::WhiteSpaceAndNewLine>(
            afterOpeningParenthesis, afterOpeningParenthesis, afterClosingParenthesis)) {
...
void TermAnalyzer::replaceTermWithMacroEquivalent(
    Looper const& eraseLooper, Looper const& parameterUsageLooper, CPlusPlusMacro& cPlusPlusMacro) {
...
void TermAnalyzer::checkMacroUsageParameters(
    ...
    if (numOfParametersInUsage != numOfParametersInDefinition) {
        addFinding(
            ...
            "The number of parameters in Macro definition does not match Macro usage", parameterLooper);
    }
}
