...
TermAnalyzer::TermAnalyzer(DequeOfTerms& terms, CPlusPlusDatabase& database, Findings& findings)
    : m_terms(terms),
      m_database(database),
      m_findings(findings),
      m_findingsBuffer(),
...
void TermAnalyzer::analyzeThisScope(Looper const& startLooper) {
    ...
    while (mainLooper.isNotFinished()) {
        DBGPRINT1(
            "analyzeThisScope file=[" << m_fileName << "] mainLooper:[" << mainLooper << " size:" << m_terms.size()
...
bool TermAnalyzer::isModifiedDueToCStyleStruct(Looper const& startLooper) {
    ...
    if (isMultiLineComparisonSatisfiedThenMoveLooper<LooperConnector::None>(compareLooper, termCheckers)) {
        ...
        combineToASingleTermWithValueType(
            startLooper, compareLooper, TermType::Class,
            m_database.getClassReferenceWithAlias(className).getTypeReference());
...
string TermAnalyzer::getLocatorString(Looper const& startLooper) {
    ...
    string locator = string("[   |Content:[") + firstTerm.getPrintableString() + "] Type:[" +
                     firstTerm.getTermTypeString() + "]|   ]";
...
string TermAnalyzer::accumulateLooperContentsToString(Looper const& startLooper) {
    ...
    return std::accumulate(
        startLooper.getCurrentIterator(), startLooper.getEndIterator(), string(""),
        [&](string stringParam, Term& term) { return stringParam + term.getString(); });
}
...
string TermAnalyzer::accumulateLooperContentsToPrintableString(Looper const& startLooper) {
    ...
    return std::accumulate(
        startLooper.getCurrentIterator(), startLooper.getEndIterator(), string(""),
        [&](string stringParam, Term& term) { return stringParam + "[" + term.getPrintableString() + "]"; });
}
