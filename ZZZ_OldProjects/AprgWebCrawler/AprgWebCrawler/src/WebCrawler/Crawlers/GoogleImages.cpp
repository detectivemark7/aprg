    ...
    if (!htmlFileStream.is_open()) {
        ...
        while (htmlFileReader.isNotFinished()) {
            ...
            if (isStringFoundInsideTheOtherStringCaseSensitive(lineInHtmlFile, R"(Download)")) {
                ...
                string fileName(
                    getStringWithUrlDecodedString(getStringInBetweenTwoStrings(lineInHtmlFile, R"(title=")", R"(>)")));
...
bool GoogleImages::areLinksInvalid() const { return m_linkForVideo.empty() || m_localPathForCurrentVideo.empty(); }
