    ...
    if (!htmlFileStream.is_open()) {
        ...
        while (htmlFileReader.isNotFinished()) {
            ...
            if (isStringFoundInsideTheOtherStringCaseSensitive(lineInHtmlFile, R"(<img id="arf-reader")")) {
                ...
                index = convertStringToNumber<unsigned int>(
                    getStringInBetweenTwoStrings(lineInHtmlFile, R"('index' : )", R"(,)"));
            } else if (isStringFoundInsideTheOtherStringCaseSensitive(lineInHtmlFile, R"('currentURL' : ')")) {
                index++;
                m_linkForNextHtml = getStringInBetweenTwoStrings(lineInHtmlFile, R"('currentURL' : ')", R"(',)") +
        ...
        m_localPathForCurrentFileToDownload = m_webCrawler.getDownloadDirectory() + R"(chapter)" +
                                              chapterWebPathHandler.getImmediateDirectoryName() + R"(\)" +
