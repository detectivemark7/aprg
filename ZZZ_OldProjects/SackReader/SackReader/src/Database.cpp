...
ParameterDetails Database::getStructureParameterDetails(
    string const& structureName, string const& parameterName) const {
...
bool Database::doesThisStructureAndParameterExistsInVector(
    string const& structureName, string const& parameterName) const {
    bool result(false);
    if (doesThisStructureExists(structureName)) {
        StructureDetails const& structureDetails = structureNameToStructureDetailsMap.at(structureName);
        vector<string> const& parametersWithCorrectOrder(structureDetails.parametersWithCorrectOrder);
        if (find(parametersWithCorrectOrder.cbegin(), parametersWithCorrectOrder.cend(), parameterName) !=
            parametersWithCorrectOrder.cend()) {
            result = true;
        }
    }
    return result;
