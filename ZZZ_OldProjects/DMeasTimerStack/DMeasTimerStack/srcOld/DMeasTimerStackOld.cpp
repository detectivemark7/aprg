...
unsigned int tcomSetSfn(unsigned int sfn) { MY_SFN = sfn; }
...
inline int TimerStack::ARRAY_timers_empty(void) { return !TimerStack::timerTableNbrOfElements; }
    ...
    do  // while( !(insertRetry < driveLoop) )
    {
        ...
        if (TimerStack::timerTableNbrOfElements < TIMER_TABLE_MAX_NBR_OF_ELEMENTS) {
                    ...
                    if (ARRAY_timers_high_mark < timerIndex) {
                        ...
                        ARRAY_timers_max_index =
                            (TIMER_TABLE_MAX_NBR_OF_ELEMENTS < (ARRAY_timers_high_mark + 1)
                                 ? TIMER_TABLE_MAX_NBR_OF_ELEMENTS
                                 : (ARRAY_timers_high_mark + 1));

                        DM_DBG_LOW(
                            "ARRAY_timers_insert: high mark: %d max index: %d", ARRAY_timers_high_mark,
                            ARRAY_timers_max_index);
                    }
                    return timerIndex;
                }
            ...
            if (0 != freeElemCount) {
                ...
                DM_INF_HIGH(
                    "Found %u free elements in stack, new count of timers %d", freeElemCount,
                    TimerStack::timerTableNbrOfElements);
...
TimerStack::TimerIndex TimerStack::ARRAY_timers_find(TimeCounter timeToSearch) {
    for (TNumberOfItems timerIndex = 0; (timerIndex < ARRAY_timers_max_index); timerIndex++) {
...
inline TimerStack::TimerIndex TimerStack::ARRAY_timers_end(void) { return TIMER_TABLE_MAX_NBR_OF_ELEMENTS; }

EBoolean TimerStack::ARRAY_timers_erase(TimerStack::TimerIndex& iter) {
    ...
    if (timers[iter].second.timerType != TimerType::FreeTimer) {
        ...
        DM_WRN_HIGH(
            "Timer %u with time %d and type %d not reserved, cannot erase", iter, timers[iter].first,
...
void TimerStack::ARRAY_timers_clear(void) {
    ...
    for (int timerIndex = 0; timerIndex < TIMER_TABLE_MAX_NBR_OF_ELEMENTS; timerIndex++) {
        timers[timerIndex].second.timerType = TimerType::FreeTimer;
    }
}

TimeCounter TimerStack::ARRAY_timers_first(TimerStack::TimerIndex iter) {
...
TimerData* TimerStack::ARRAY_timers_second(TimerStack::TimerIndex iter) {
    if (!ARRAY_timers_empty()) {
        if (iter < TIMER_TABLE_MAX_NBR_OF_ELEMENTS) {
...
TimerStack::TimerIndex TimerStack::ARRAY_timers_next(TimerStack::TimerIndex elem) {
    if (elem < TIMER_TABLE_MAX_NBR_OF_ELEMENTS) {
        for (u32 timerIndex = 0; timerIndex < TIMER_TABLE_MAX_NBR_OF_ELEMENTS; timerIndex++) {
...
void TimerStack::trigExpired(TimerIndex expired) {
    ...
    switch (timerType) {
        ...
        case TimerType::SlaveMeasurementInit:
            MTPRINTF("*******************************************\n");
            DM_DBG_LOW(
                "Expired - %u/%u - Measurement: %u / %u (%u) - Sicad: %#X", expired, timeCounter,
        ...
        case TimerType::NbccRecovery:
            MTPRINTF("*******************************************\n");
            DM_DBG_LOW("Expired - %u/%u - NbccRecovery. No Reset to RNC", expired, timeCounter);
            MTPRINTF("*******************************************\n");
        ...
        case TimerType::DefinitelyRemoveUser:
            DM_DBG_LOW(
                "Expired - %u/%u - DefinitelyRemoveUser, nbccId: %u", expired, timeCounter, timerData->value.nbccid);
...
EBoolean TimerStack::preRefresh(const TSfn newSfn) {
    TSfn tmpSfn;
...
EBoolean TimerStack::insert(const TSfn sfn, const TimerData& timerData) {
...
void TimerStack::initIterator(TimerIndex& iter) { iter = ARRAY_timers_end(); }

EBoolean TimerStack::isValidIterator(const TimerIndex& iter) {
...
EBoolean TimerStack::insertMeasurement(
    ...
    const TUserId userId, TimerIndex& iter, TAaSysComSicad const sicadToWait, EFaultId const faultId) {
    TimerData timerData;
    timerData.timerType = timerType;
...
EBoolean TimerStack::insert(TimeCounter timeValue, TimerData const& timerData, TimerIndex& iter) {
    ...
    DM_DBG_LOW("timeValue: %u,  DMeas::currSfn: %u, realSfn: %u", timeValue, DMeas::currSfn, realSfn);
...
EBoolean TimerStack::renewMeasurement(TimerIndex& iter, TimeCounter const timeAdvance) {
 ...
 *
 */
EBoolean TimerStack::remove(TimerIndex& iter) {
    EBoolean retStatus = EBoolean_False;
    if (iter != ARRAY_timers_end()) {
...
EBoolean TimerStack::trigExpiredAndRemove(TimerIndex& timerIter) {
...
TimerData* TimerStack::timersFindNbccRecovery(TNodeBCommunicationContextId nbccId, TimerIndex& iter) {
    initIterator(iter);
    if (!ARRAY_timers_empty()) {
...
void TimerStack::dump(TimerIndex begin, TimerIndex end) {
    ...
    for (u32 counter = 0; counter < 10; counter++) {
        if (EBoolean_True == isValidIterator(iter)) {
            ...
            MTPRINTF(
                "TimersTable[%02d]:    %-4U | %s | MId: %d / nbccId: %u\n", iter, timers[iter].first,
                timerTypeStr.c_str(), timers[iter].second.value.measurement.measurementId,
