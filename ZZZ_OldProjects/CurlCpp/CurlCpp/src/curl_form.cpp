...
curl_form &curl_form::operator=(const curl_form &form) {
    ...
    while (old_head != nullptr) {
        if (this->form_post == nullptr) {
            this->is_null(
                this->last_ptr = this->form_post = (struct curl_httppost *)malloc(sizeof(struct curl_httppost)));
...
void curl_form::add(
    const curl_pair<CURLformoption, string> &form_name, const curl_pair<CURLformoption, string> &form_content) {
    if (curl_formadd(
            &this->form_post, &this->last_ptr, form_name.first(), form_name.second(), form_content.first(),
...
void curl_form::add(
    ...
    if (curl_formadd(
            &this->form_post, &this->last_ptr, form_name.first(), form_name.second(), form_content.first(),
            form_content.second(), content_type.first(), content_type.second(), CURLFORM_END) != 0) {
        throw curl_exception("Error while adding the form", __FUNCTION__);
    }
}
...
void curl_form::add(
    ...
    if (curl_formadd(
            &this->form_post, &this->last_ptr, form_name.first(), form_name.second(), form_content.first(),
            form_content.second(), content_length.first(), content_length.second(), CURLFORM_END) != 0) {
        throw curl_exception("Error while adding the form", __FUNCTION__);
    }
}
...
void curl_form::add(
    const curl_pair<CURLformoption, string> &form_name, const curl_pair<CURLformoption, int> &name_length,
    const curl_pair<CURLformoption, string> &form_content) {
    if (curl_formadd(
            &this->form_post, &this->last_ptr, form_name.first(), form_name.second(), form_content.first(),
            form_content.second(), name_length.first(), name_length.second(), CURLFORM_END) != 0) {
        throw curl_exception("Error while adding the form", __FUNCTION__);
    }
}
...
void curl_form::add(
    const curl_pair<CURLformoption, string> &form_name, const curl_pair<CURLformoption, string> &form_content,
    const curl_pair<CURLformoption, int> &content_length, const curl_pair<CURLformoption, string> &content_type) {
    if (curl_formadd(
            &this->form_post, &this->last_ptr, form_name.first(), form_name.second(), form_content.first(),
            form_content.second(), content_length.first(), content_length.second(), content_type.first(),
...
void curl_form::add(
    const curl_pair<CURLformoption, string> &form_name, const curl_pair<CURLformoption, string> &form_bufname,
    const curl_pair<CURLformoption, char *> &form_content, const curl_pair<CURLformoption, long> &content_length) {
    if (curl_formadd(
            &this->form_post, &this->last_ptr, form_name.first(), form_name.second(), form_bufname.first(),
            form_bufname.second(), form_content.first(), form_content.second(), content_length.first(),
...
void curl_form::add(const curl_pair<CURLformoption, string> &form_name, const vector<string> &files) {
    ...
    if (curl_formadd(
            &this->form_post, &this->last_ptr, form_name.first(), form_name.second(), CURLFORM_ARRAY, new_files,
            CURLFORM_END) != 0) {
        delete[] new_files;
