 ...
 */
template <typename t>
void cpMatlabData(const t *mdata) {
    if (cimg::type<t>::is_float()) {
        cimg_forXYZC(*this, x, y, z, v) (*this)(x, y, z, v) = (T)(mdata[((v * _depth + z) * _width + x) * _height + y]);
    } else {
        cimg_forXYZC(*this, x, y, z, v) (*this)(x, y, z, v) =
            (T)(int)(mdata[((v * _depth + z) * _width + x) * _height + y]);
    }
}
 ...
 */
CImg(const mxArray *matlabArray, const bool vdata = false) : _is_shared(false) {
    ...
    if (nbdims > 4 || !isNumericalClassID(classID)) {
        ...
        if (nbdims == 4) {
            _depth = (unsigned)dims[2];
            _spectrum = (unsigned)dims[3];
        } else if (nbdims == 3) {
            if (vdata)
                _spectrum = (unsigned)dims[2];
            else
                _depth = (unsigned)dims[2];
        }
        _data = new T[size()];
...
CImg &operator=(const mxArray *matlabArray) {
    int nbdims = (int)mxGetNumberOfDimensions(matlabArray), classID = mxGetClassID(matlabArray);
    if (nbdims > 4 || !isNumericalClassID(classID)) {
        delete[] _data;
        _data = 0;
        _width = _height = _depth = _spectrum = 0;
 ...
 */
template <typename c>
void populate_maltlab_array(c *const mdata) const {
...
void populate_maltlab_array(mxLogical *const mdata) const {
    cimg_forXYZC(*this, x, y, z, v) mdata[((v * _depth + z) * _width + x) * _height + y] =
        (mxLogical)((*this)(x, y, z, v) != 0);
}
