...
CImg(const cv::Mat& src) : _width(0), _height(0), _depth(0), _spectrum(0), _is_shared(false), _data(0) { assign(src); }
        ...
        switch (src.depth()) {
            ...
            case CV_8U:  // 8-bit unsigned int.
                if (src.channels() == 1) {
                    ...
                    CImg<ucharT> tmp(src.cols, src.rows, 1, 3),
                        R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
            ...
            case CV_16U:  // 16-bit unsigned int.
                if (src.channels() == 1) {
                    ...
                    std::vector<cv::Mat> channels;
                    cv::split(src, channels);
                    CImg<ushortT> tmp(src.cols, src.rows, 1, 3),
                        R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
            ...
            case CV_16S:  // 16-bit signed int.
                if (src.channels() == 1) {
                    ...
                    std::vector<cv::Mat> channels;
                    cv::split(src, channels);
                    CImg<shortT> tmp(src.cols, src.rows, 1, 3),
                        R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
            ...
            case CV_32S:  // 32-bit signed int.
                if (src.channels() == 1) {
                    ...
                    std::vector<cv::Mat> channels;
                    cv::split(src, channels);
                    CImg<intT> tmp(src.cols, src.rows, 1, 3),
                        R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
            ...
            case CV_32F:  // 32-bit float.
                if (src.channels() == 1) {
                    ...
                    std::vector<cv::Mat> channels;
                    cv::split(src, channels);
                    CImg<floatT> tmp(src.cols, src.rows, 1, 3),
                        R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
            ...
            case CV_64F:  // 64-bit double.
                if (src.channels() == 1) {
                    ...
                    std::vector<cv::Mat> channels;
                    cv::split(src, channels);
                    CImg<doubleT> tmp(src.cols, src.rows, 1, 3),
                        R = tmp.get_shared_channel(2), G = tmp.get_shared_channel(1), B = tmp.get_shared_channel(0);
            ...
            default:
                throw CImgInstanceException(
                    _cimg_instance "assign(const cv::Mat&) : Mat depth is invalid.", cimg_instance);
                break;
        }
    } else {
...
cv::Mat get_MAT(const unsigned int z = 0) const {
    if (is_empty()) throw CImgInstanceException(_cimg_instance "get_MAT() : instance image is empty.", cimg_instance);
    if (z >= _depth)
        throw CImgInstanceException(
            _cimg_instance "get_MAT() : specified slice %u is out of image bounds.", cimg_instance, z);
    const CImg<T> _slice = _depth > 1 ? get_slice(z) : CImg<T>(), &slice = _depth > 1 ? _slice : *this;
    CImg<T> buf(slice, true);
    int cols = buf.width(), rows = buf.height(), nchannels = buf.spectrum(), matType = -1;
    ...
    if (matType < 0)
        throw CImgInstanceException(
            _cimg_instance "get_MAT() : pixel type '%s' is not supported.", cimg_instance, buf.pixel_type());
    cv::Mat out;
