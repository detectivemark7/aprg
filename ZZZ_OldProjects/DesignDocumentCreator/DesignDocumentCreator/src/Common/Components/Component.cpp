...
Component::Component(ComponentName const componentName)
    : m_componentName(componentName), m_componentNameInString(convertToString(componentName)) {}

void Component::pushBackEvent(Event const& event) { m_eventQueue.push_back(event); }
...
bool Component::isEventQueueEmpty() const { return m_eventQueue.empty(); }
...
ComponentName Component::getComponentName() const { return m_componentName; }

string Component::getComponentNameInString() const { return m_componentNameInString; }

string Component::getQueueAsString() const {
    return accumulate(
        m_eventQueue.begin(), m_eventQueue.end(), string("Event Queue: "),
