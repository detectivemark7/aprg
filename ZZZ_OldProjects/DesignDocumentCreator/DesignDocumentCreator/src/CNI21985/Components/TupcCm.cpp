...
TupcCm::TupcCm(ComponentName const componentName) : Component(componentName) {}

void TupcCm::handleTupcExeStartup() {
    logNoteOnComponent(
        "TUPC/CM is **not_automatically**  started when TUPCexe starts.\nThis is same with legacy Conman.");
}

void TupcCm::handleTupcCmStartup() { logNoteOnComponent("TUPC/CM is **started**."); }
...
void TupcCm::handleCmBearersModifyReqMsg(GenericMessage const& genericMessage) {
    SpecificDynamicArrayMessage<MessageName::TUPC_CM_BEARERS_MODIFY_REQ_MSG> cmBearersModifyReqMessage(
        convertGenericToSpecificDynamicArray<MessageName::TUPC_CM_BEARERS_MODIFY_REQ_MSG>(genericMessage));
    ...
    if (cmBearersModifyReqPayload.transactionType == ECmBearersModifyTransactionType_ModificationPrepare ||
        ...
        logNoteOnComponent(
            "TUPC/CM uses legacy logic in TUPC Conman to determine if MOD is needed to be sent transport based from "
            "transactionType and change in bit rate.");
...
void TupcCm::performErqEcfMessagingToTransport() {
    logNoteOnComponents(
        ComponentNames{ComponentName::TupcCm, ComponentName::Trsw},
        "TUPC/CM handles IPCS messaging to Transport (ERQ/ECF). \nPlease see Note 1");
}

void TupcCm::performRelRlcMessagingToTransport() {
    logNoteOnComponents(
        ComponentNames{ComponentName::TupcCm, ComponentName::Trsw},
        "TUPC/CM handles IPCS messaging to Transport (REL/RLC). \nPlease see Note 1");
}

void TupcCm::performModMoaMessagingToTransport() {
    logNoteOnComponents(
        ComponentNames{ComponentName::TupcCm, ComponentName::Trsw},
        "TUPC/CM handles IPCS messaging to Transport (MOD/MOA). \nPlease see Note 1");
}
...
void TupcCm::sendCmBearersSetupRespBasedCmBearersSetupReq(GenericMessage const& genericMessage) const {
    ...
    SpecificDynamicArrayMessage<MessageName::TUPC_CM_BEARERS_SETUP_REQ_MSG> cmBearersSetupReqMessage(
        convertGenericToSpecificDynamicArray<MessageName::TUPC_CM_BEARERS_SETUP_REQ_MSG>(genericMessage));
...
void TupcCm::sendCmBearersReleaseRespBasedCmBearersReleaseReq(GenericMessage const& genericMessage) const {
    ...
    SpecificDynamicArrayMessage<MessageName::TUPC_CM_BEARERS_RELEASE_REQ_MSG> cmBearersReleaseReqMessage(
        convertGenericToSpecificDynamicArray<MessageName::TUPC_CM_BEARERS_RELEASE_REQ_MSG>(genericMessage));
