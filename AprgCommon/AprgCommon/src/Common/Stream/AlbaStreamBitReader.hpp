...
public:
    ...
    std::bitset<BITSET_SIZE> readBitsetData(unsigned int const startBitsetIndex, unsigned int const endBitsetIndex);
...
std::bitset<BITSET_SIZE> AlbaStreamBitReader::readBitsetData(
    unsigned int const startBitsetIndex, unsigned int const endBitsetIndex) {
    std::bitset<BITSET_SIZE> result;
    unsigned int const numberOfBitsToRead =
        std::min(endBitsetIndex - startBitsetIndex + 1, static_cast<unsigned int>(BITSET_SIZE));
