...
// reason.
// -> Compilers typically issue warnings on such uses. The string-literal, if specified, is usually included in the
// warnings.
// -> This attribute is allowed in declarations of the following names or entities:
// ---> class/struct/union: struct [[deprecated]] S;,
// ---> typedef-name, including those declared by alias declaration: [[deprecated]] typedef S* PS;, using PS
// [[deprecated]] = S*;,
...
// ---> typedef, including those declared by alias declaration: [[maybe_unused]] typedef S* PS;, using PS
// [[maybe_unused]] = S*;,
...
// compiler that warns on fallthrough. Syntax
// -> [[fallthrough]]
// Explanation
...
// -> If the fallthrough statement is inside a loop, the next (labeled) statement must be part of the same iteration of
// that loop.
// -> Indicates that the fall through from the previous case label is intentional and should not be diagnosed by a
...
// -> This attribute must appear on the first declaration of a function or one of its parameters in any translation
// unit.
...
// ---> including that statement are more likely than any alternative path of execution that does not include such a
// statement.
// -> 2) Applies to a statement to allow the compiler to optimize for the case where paths of execution
// ---> including that statement are less likely than any alternative path of execution that does not include such a
// statement.
