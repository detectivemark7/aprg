...
AlbaMemoryBuffer::AlbaMemoryBuffer(void const* sourcePointer, unsigned int const size) { addData(sourcePointer, size); }

AlbaMemoryBuffer::operator bool() const { return hasContent(); }

bool AlbaMemoryBuffer::hasContent() const { return !m_buffer.empty(); }

unsigned int AlbaMemoryBuffer::getSize() const { return m_buffer.size(); }
...
void AlbaMemoryBuffer::resize(unsigned int const size) { m_buffer.resize(size); }
