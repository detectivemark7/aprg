...
// -> On many platforms (an exception is systems with segmented addressing) std::size_t can safely store the value of
...
// Construct: offsetof
//
// The macro offsetof expands to an integral constant expression of type std::size_t, the value of which is the offset,
// in bytes, from the beginning of an object of specified type to its specified subobject, including padding if any.
// Given an object o of type type and static storage duration, o.member shall be an lvalue constant expression that
// refers to a subobject of o. Otherwise, the behavior is undefined. Particularly, if member is a static data member, a
// bit-field, or a member function, the behavior is undefined. If type is not a PODType (until C++11)standard layout
// type (since C++11), the behavior is undefined (until C++17) use of the offsetof macro is conditionally-supported
// (since C++17). The expression offsetof(type, member) is never type-dependent and it is value-dependent if and only if
// type is dependent.
//
// Example:
// -> #include <iostream>
...
// alignas( type-id )
// alignas( pack ... )
// 1) alignas(expression) must be an integral constant expression that evaluates to zero, or to a valid value for an
// alignment or extended alignment. 2) Equivalent to alignas(alignof(type)) 3) Equivalent to multiple alignas specifiers
// applied to the same declaration, one for each member of the parameter pack, which can be either type or non-type
// parameter pack.
//
// Explanation:
...
// (largest) non-zero expression of all alignas specifiers used in the declaration, unless it would weaken the natural
// alignment of the type.
//
// If the strictest (largest) alignas on a declaration is weaker than the alignment it would have without any alignas
// specifiers (that is, weaker than its natural alignment or weaker than alignas on another declaration of the same
// object or type), the program is ill-formed:
//
// struct alignas(8) S {};
...
// which is either complete object type, an array type whose element type is complete, or a reference type to one of
// those types.
//
// If the type is reference type, the operator returns the alignment of referenced type; if the type is array type,
// alignment requirement of the element type is returned.
//
// Example:
// -> #include <iostream>
// ->
