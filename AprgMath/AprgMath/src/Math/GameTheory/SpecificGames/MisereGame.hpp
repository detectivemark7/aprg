...
public:
    ...
    // The idea is to first play the misère game like the standard nim game, but change the strategy at the end of the game.
    ...
    // This strategy works because a state where the strategy changes always appears in the game, and this state is a winning state,
    // because it contains exactly one heap that has more than one stick so the nim sum is not 0.
