...
GrundyGame::GrundyGame(
        UnsignedInteger const totalNumberOfSticks)
    : m_stickHeaps{totalNumberOfSticks}
{}
...
GrundyGame::StickHeaps const& GrundyGame::getStickHeaps() const
{
    return m_stickHeaps;
}
...
GameState GrundyGame::getGameState()
{
    ...
    else if(GameState::Winning == gameState)
    {
        ...
        for(UnsignedInteger heapIndex=0; !isFound && heapIndex<m_stickHeaps.size(); heapIndex++)
        {
            ...
            for(int a=(static_cast<int>(stickHeap)-1)/2; !isFound && a>0; a--)
            {
                ...
                if(hammingDistance == getCombinedGrundyNumber(getGrundyNumberWithNumberOfSticks(a), getGrundyNumberWithNumberOfSticks(b)))
                {
...
void GrundyGame::split(
        HeapIndexAndFirstPileAndSecondPile const& heapIndexAndFirstPileAndSecondPile)
{
...
SetOfUnsignedIntegers GrundyGame::getNextGrundyNumbersWithNumberOfSticks(UnsignedInteger const numberOfSticks)
{
    ...
    for(unsigned int a=1; a<limit; a++)
    {
        ...
        result.emplace(getCombinedGrundyNumber(getGrundyNumberWithNumberOfSticks(a), getGrundyNumberWithNumberOfSticks(b)));
    }
    return result;
}
