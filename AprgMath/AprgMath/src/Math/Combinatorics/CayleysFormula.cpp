...
UnsignedInteger getNumberOfLabeledTreesUsingCayleysFormula(UnsignedInteger const& numberOfNodes) {
    ...
    // At each step, the leaf with the smallest label is removed, and the label of its only neighbor is added to the
    // code. We can construct a Prüfer code for any tree, and more importantly, the original tree can be reconstructed
    // from a Prüfer code.
