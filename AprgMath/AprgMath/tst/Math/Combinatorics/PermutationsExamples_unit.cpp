...
TEST(PermutationsExamplesTest, BasicExamplesWorks) {
    ...
    // If four people enter a bus in which there are ten vacant seats, how many ways are there for the four to be
    // seated? Solution: We can convert this problem to position of the four people. [Position of first person][Position
    // of second person][Position of third person][Position of fourth person] This reduces to permutations of 10
    // taken 4. This is the same SeatsWithPeople problem (dicussion is on the next tests)
    ...
    // All permutations: [Permutations for "1234567890"] / [Permutations to cancel out "AAAA"] / [Permutations to cancel
    // out "BBB"] / [Permutations to cancel out "CC"]
    ...
    // All permutations: [Permutations for "m123456789A"] / ([Permutations to cancel out "iiii"] * [Permutations to
    // cancel out "ssss"] * [Permutations to cancel out "pp"])
    EXPECT_EQ(34650U, getFactorial(11U) / (getFactorial(4U) * getFactorial(4U) * getFactorial(2U)));

    // How many ways can four boys and three girls be seated in a row containing seven seats if the boys and girls must
    // alternate? Solution: Split the problem in to two parts (boys and girls) Permutations for boys: Permutations of 4
    // taken 4 Permutations for girls: Permutations of 3 taken 3 All permutations: [Permutations for boys] *
    // [Permutations for girls]
    ...
    // -> This is true because these loop around permutations are equal in circular table: {1,2,3,4} {2,3,4,1} {3,4,1,2}
    // {4,1,2,3} All permutations: [Permutations for linear table] / [Permutations to cancel out for loop around]
    EXPECT_EQ(6U, getFactorial(4U) / 4U);

    // What is the number of permutations of the characters in LILAC so that no character appears in its original
    // position, if the two L’s are indistinguishable? There are 3 choices for the first slot, and then 2 for the third
    // slot. That leaves one letter out of I,A,C unchosen and there are 2 slots that one might occupy. After that, the
    // L′s must go in the 2 unfilled slots. Hence the answer is, 3×2×1×2 = 12
...
TEST(PermutationsExamplesTest, SeatsWithPeopleExampleScenario2Works) {
    ...
    // we put a person in to the seat, and the symbol '>' means that we move to the next seat to the right. Using this
